...
unsigned int cell_number = 0; // global variable which is necessary for the visited set on the bfs
...
struct Vertex {
    ...
    */
    ...
    Vertex* U;
    Vertex* D;
    unsigned int cell; // to identify and to use on the visited set later
...
// helper functions
void write_to_tuple(std::ifstream& file, std::tuple<unsigned int, unsigned int, unsigned int>& tuple_to_add){
...
void find_path_bfs_helper(Vertex current_vertex, std::vector<bool>& set_visited, std::vector<std::string> current_path, std::string destination, std::queue<std::pair<Vertex, std::vector<std::string>>>& queue){
    ...
    */
   if (set_visited[current_vertex.cell] == true) {
        return;
   }
...
std::vector<std::string> find_path_bfs(Vertex start, Vertex end){
    ...
    */
    ...
    while (!queue.empty()) {
        ...
        // a vertex can have multiple edges/pointers hence an if-if is needed
        if (vertex.N != nullptr){
...
std::vector<std::vector<std::vector<Vertex>>> Graph(std::tuple <unsigned int, unsigned int, unsigned int> dimensions, std::ifstream& read_file){
    /*
    ...
    */
    ...
    for (unsigned int z = 0; z < (std::get<0>(dimensions)*std::get<1>(dimensions)); z = z + std::get<1>(dimensions)){ // traverses until the number of (rows * levels) is reached
        ...
        for (unsigned int y = 0; y < std::get<1>(dimensions); y++){ // loops until number of rows
            ...
            for (unsigned int x = 0; x < std::get<2>(dimensions); x++){ // loops until number of columns
    ...
    for (unsigned int z = 0; z < helper_graph.size(); z++){
        for (unsigned int y = 0; y < helper_graph[z].size(); y++){
            for (unsigned int x = 0; x < helper_graph[z][y].size(); x++){
                ...
                for (unsigned int w = 0; w < current_string.length(); w++){
...
int main(){
    ...
    std::tuple <unsigned int, unsigned int, unsigned int> size; // levels, rows, columns
    std::tuple <unsigned int, unsigned int, unsigned int> start; // coordinates for the starting vertex
    std::tuple <unsigned int, unsigned int, unsigned int> end; // coordinates for the destination vertex
    ...
    std::vector<std::string> path_start_to_end = find_path_bfs(graph[std::get<0>(start)][std::get<1>(start)][std::get<2>(start)], graph[std::get<0>(end)][std::get<1>(end)][std::get<2>(end)]);
    write_to_file(path_start_to_end);
